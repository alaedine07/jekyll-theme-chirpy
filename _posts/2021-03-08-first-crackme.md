...
what all this people will try to do is __reverse engineering__ your program. they will try to understand what he is doing based on the input/output and using other tools made for this purpose.

Reversing is an __art__
...
now let's go back to the line +53. the instruction ```mov    rax,QWORD PTR [rbp-0x20]``` means the it will move something from the stack to the **rax** register(register are the memory space used directly by the processor). after few other instructions it will call the function
...
we can see that the buffer on the stack(variable) used to sum which is *rbp-0x8* is getting the value **0x41** after the 1st iteration which is 61 in decimal which corresponds to *A* in ASCII and **0x83**(131 in decimal) after the second iteration which is 65 + 66 which corresponds to ascii(A) + ascii(B). i'm using the command 
